...
class DD_robot(object):
    ...
    def generate_train_profiles_pp(self, ref_speed = 2.00, K = 2.00, waypoints = None, tol = 0.05, sigma = None):
        '''
        Generate training path, training velocity and training acceleration for the differential drive robot. 
        ...
        while goal_dist > tol: # and i < max_iter:
            ...
            currentPos = [float(self.X[0]), float(self.X[1])]
            currentHeading = float(self.X[2])
            goalPt, lastIndex, omega = pp.pure_pursuit_step(waypoints, currentPos, currentHeading, lookAheadDis, lastFoundIndex)
